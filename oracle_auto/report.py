...
def deployment_table(config: AutomationConfig) -> str:
    rows = [
        ...
        ("DNS Model", "SCAN via DNS unless scan_ip/scan_ips is set; public/private/VIP are managed in /etc/hosts"),
...
def topology_table(config: AutomationConfig) -> str:
    ...
    for site in config.sites:
        for node in site.nodes:
            rows.append(
                ...
                f"<td>{html.escape(_scan_cell(site))}</td>"
    ...
    return (
        "<table><thead><tr><th>Site</th><th>DB Unique Name</th><th>SCAN</th>"
...
def scan_table(config: AutomationConfig) -> str:
    ...
    for site in config.sites:
        rows.append(
            ...
            f"<td>{html.escape(_scan_validation_source(site))}</td>"
            "</tr>"
        )
    return "<table><thead><tr><th>Site</th><th>SCAN Name</th><th>Validation Source</th></tr></thead><tbody>" + "".join(rows) + "</tbody></table>"


def _scan_cell(site: SiteConfig) -> str:
    if not site.scan_name:
        return ""
    if site.scan_ips:
        return f"{site.scan_name} ({', '.join(site.scan_ips)})"
    return site.scan_name


def _scan_validation_source(site: SiteConfig) -> str:
    if not site.scan_name:
        return "not required"
    if site.scan_ips:
        return "/etc/hosts generated by prepare-os"
    return "DNS required"
