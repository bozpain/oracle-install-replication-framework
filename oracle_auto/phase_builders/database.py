...
def _stale_dbca_cleanup_lines(db_name: str, unique: str) -> list[str]:
    ...
    return [
        ...
        f"if ! sudo -iu oracle {DB_HOME}/bin/srvctl config database -db {quoted_unique} >/dev/null 2>&1 && ! sudo -iu oracle {DB_HOME}/bin/srvctl config database -db {quoted_db_name} >/dev/null 2>&1; then",
        f"  if ps -eo args= | sed 's/[[:space:]].*//' | grep -qx \"ora_pmon_{unique}\"; then",
