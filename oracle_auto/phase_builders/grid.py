...
def _grid_config_tools_script(config: AutomationConfig, site: SiteConfig) -> str:
    ...
    lines = [
        ...
        *_grid_known_hosts_lines(site),
        ...
        "    latest_grid_log_dir=$(ls -td /u01/app/oraInventory/logs/GridSetupActions* 2>/dev/null | head -1 || true)",
        f"    for log_root in \"$latest_grid_log_dir\" {GRID_BASE}/cfgtoollogs; do",
        "      test -n \"$log_root\" && test -d \"$log_root\" || continue",
        "      find \"$log_root\" -type f \\( -name '*.log' -o -name '*.out' -o -name '*.err' \\) -mmin -120 -print0 2>/dev/null |",
